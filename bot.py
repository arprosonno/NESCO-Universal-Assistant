...
# Semaphore to prevent scheduler pile-up
...
async def fetch_nesco_balance(account: str) -> float:
    async with async_playwright() as p:
        ...
        try:
            ...
            # Wait for the balance text to appear
            await page.wait_for_selector("text=অবশিষ্ট ব্যালেন্স (টাকা)", timeout=15_000)
            text = await page.inner_text("body")

        finally:
            await browser.close()

    # Regex to find "অবশিষ্ট ব্যালেন্স (টাকা)" followed by numbers
    match = re.search(r"অবশিষ্ট ব্যালেন্স \(টাকা\)\s*[:\-]?\s*([\d,]+\.\d+)", text)
    if not match:
...
async def fetch_with_retry(account: str) -> float:
    ...
    async with FETCH_SEMAPHORE:
        for _ in range(RETRY_ATTEMPTS):
            try:
                return await asyncio.wait_for(fetch_nesco_balance(account), timeout=FETCH_TIMEOUT)
...
async def broadcast(context: ContextTypes.DEFAULT_TYPE, title: str):
    for chat_id, data in list(USER_DATA.items()):
        ...
        try:
            ...
            await context.bot.send_message(chat_id, f"{title}\n💡 Remaining Balance: *{bal:.2f} BDT*", parse_mode="Markdown")
        except Exception:
            continue
    ...
    try:
        for chat_id, data in list(USER_DATA.items()):
            ...
            if bal < LOW_BALANCE_THRESHOLD:
                await context.bot.send_message(chat_id, f"🚨 *LOW BALANCE ALERT!*\nRemaining: {bal:.2f} BDT", parse_mode="Markdown")
    except Exception:
        pass
